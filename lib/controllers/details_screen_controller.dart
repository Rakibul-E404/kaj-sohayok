...
///
///
/// todo:: upper is imtiaz vai's code---->>
///
///
///
///
...
class DetailsScreenController extends GetxController {
  ...
  // 🔴 ADDED: Safe number conversion helpers
  ...
  int? _safeToInt(dynamic value) {
    ...
    log('⚠️ Unexpected type for int conversion: ${value.runtimeType}');
    return null;
  }

  /// Safely converts dynamic number to double
  /// Handles both int and double from API responses
  double? _safeToDouble(dynamic value) {
    ...
    log('⚠️ Unexpected type for double conversion: ${value.runtimeType}');
    return null;
  }
  ...
  // 🔴 FIXED: Updated _parseResult with safe number conversions
  Result _parseResult(Map<String, dynamic> data) {
    try {
      log('🔍 Parsing Result data...');
      log('   Raw startPrice: ${data['startPrice']} (${data['startPrice']?.runtimeType})');
      log('   Raw rating: ${data['rating']} (${data['rating']?.runtimeType})');
      log('   Raw yearsOfExperience: ${data['yearsOfExperience']} (${data['yearsOfExperience']?.runtimeType})');

      // print("asasdfasdfasda; ${data}");
      // return Result();
      ...
      return Result(
        ...
        attachmentsForCoverPhoto:
            data['attachmentsForCoverPhoto'] as List<dynamic>?,
        yearsOfExperience: _safeToInt(data['yearsOfExperience']),
        // 🔴 FIXED: Safe conversion
        serviceProviderId: data['_ServiceProviderId'] as String?,
      );
    } catch (e, stackTrace) {
      log('❌ Error in _parseResult: $e');
      log('   Stack trace: $stackTrace');
      log('   Data: $data');
      rethrow;
    }
  ...
  // 🔴 FIXED: Updated _parseReview with safe number conversions
  Review _parseReview(Map<String, dynamic> data) {
    try {
      final userIdData = data['userId'] as Map<String, dynamic>?;
      ...
      return Review(
        ...
        updatedAt: data['updatedAt'] != null
            ? DateTime.tryParse(data['updatedAt'] as String)
            : null,
        v: _safeToInt(data['__v']),
        // 🔴 FIXED: Safe conversion
        reviewId: data['_ReviewId'] as String?,
      );
    } catch (e, stackTrace) {
      log('❌ Error in _parseReview: $e');
      log('   Stack trace: $stackTrace');
      log('   Data: $data');
      rethrow;
    }
  ...
  // 🔴 FIXED: Updated _parseFullResult with safe number conversions
  FullResult _parseFullResult(Map<String, dynamic> data) {
    try {
      return FullResult(
        rating: _safeToInt(data['rating']), // 🔴 FIXED: Safe conversion
        count: _safeToInt(data['count']), // 🔴 FIXED: Safe conversion
      );
    } catch (e, stackTrace) {
      log('❌ Error in _parseFullResult: $e');
      log('   Stack trace: $stackTrace');
      log('   Data: $data');
      rethrow;
    }
