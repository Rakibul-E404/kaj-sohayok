...
class SvpSubmitWorkFormScreenController extends GetxController {
  ...
  void _parseServiceBookingData(
    ...
    try {
      ...
      // FIXED: Clear and update additional costs properly
      additionalCosts.clear();
      if (additionalCostsData != null && additionalCostsData is List) {
        final List<AdditionalCostModel> tempList = [];
        for (final cost in additionalCostsData) {
          if (cost is Map<String, dynamic>) {
            ...
            tempList.add(AdditionalCostModel(
                title: costName,
                price: costPrice,
                id: additionalCostId
            ));
        ...
        // Use assignAll to properly update the observable list
        additionalCosts.assignAll(tempList);
      }

      log("📊 Additional costs loaded: ${additionalCosts.length} items");
      update();
    } catch (e, stackTrace) {
  ...
  bool _isCompletionDateValid(DateTime selectedDate) {
    ...
    final bookingDateOnly = DateTime(bookingDate.value!.year, bookingDate.value!.month, bookingDate.value!.day);

    // Check if completion date is BEFORE booking date (same day is allowed)
    if (selectedDateOnly.isBefore(bookingDateOnly)) {  // ← Compare date-only values
    ...
    final daysDifference = selectedDateOnly.difference(bookingDateOnly).inDays;
  ...
  // FIXED: Method to remove additional cost by ID
  void removeAdditionalCostById(String id) {
    log("🗑️ Attempting to remove additional cost with ID: $id");

    if (id.isEmpty || id == 'null') {
      // If no ID, just refresh from server
      loadWorkDetails();
      return;
    }

    final initialLength = additionalCosts.length;
    additionalCosts.removeWhere((item) => item.id == id);
    final finalLength = additionalCosts.length;

    if (initialLength != finalLength) {
      additionalCosts.refresh();
      log("🗑️ Removed additional cost with ID: $id");
    } else {
      log("⚠️ No cost found with ID: $id, refreshing from server");
      loadWorkDetails();
    }

    log("📊 Remaining additional costs: ${additionalCosts.length}");
  }

  // NEW: Method to handle additional cost deletion properly
  Future<void> handleAdditionalCostDeleted(String additionalCostId) async {
    log("🔄 Handling additional cost deletion for ID: $additionalCostId");

    if (additionalCostId.isEmpty || additionalCostId == 'null') {
      // If no ID, just refresh the entire list
    ...
    final initialLength = additionalCosts.length;
    additionalCosts.removeWhere((item) => item.id == additionalCostId);

    if (initialLength != additionalCosts.length) {
      additionalCosts.refresh();
      log("✅ Successfully removed cost with ID: $additionalCostId from local list");
    } else {
      log("⚠️ Cost not found locally with ID: $additionalCostId");
    }
  ...
  // FIXED: Load work details method
  ...
  // FIXED: Add additional cost method with proper ID handling
  Future<bool> addAdditionalCost(String name, double price) async {
    ...
    try {
      ...
      if (response.isSuccess) {
        // FIX: Check different response structures for ID
        String? additionalCostId;

        // Try multiple possible paths for the ID
        if (response.jsonResponse?['data']?['id'] != null) {
          additionalCostId = response.jsonResponse!['data']['id'].toString();
        } else if (response.jsonResponse?['data']?['attributes']?['_AdditionalCostId'] != null) {
          additionalCostId = response.jsonResponse!['data']['attributes']['_AdditionalCostId'].toString();
        } else if (response.jsonResponse?['_AdditionalCostId'] != null) {
          additionalCostId = response.jsonResponse!['_AdditionalCostId'].toString();
        } else if (response.jsonResponse?['data']?['attributes']?['additionalCostId'] != null) {
          additionalCostId = response.jsonResponse!['data']['attributes']['additionalCostId'].toString();
        }

        log("📌 Additional cost ID from response: $additionalCostId");

        // Add to observable list with the new ID
        additionalCosts.add(AdditionalCostModel(
            title: name,
            price: price,
            id: additionalCostId
        ));
        ...
        // Force UI update
        additionalCosts.refresh();
        log("✅ Additional cost added successfully with ID: $additionalCostId");

        // Immediately refresh from server to get consistent state
  ...
  // Remove additional cost by index (for local operations)
  void removeAdditionalCost(int index) {
    if (index >= 0 && index < additionalCosts.length) {
      final item = additionalCosts[index];
      log("🗑️ Removing additional cost at index $index: ${item.title}");
      additionalCosts.removeAt(index);
      additionalCosts.refresh(); // Force UI update
    }
  }
