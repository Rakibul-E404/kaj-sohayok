...
class SvpInProgressScreenController extends GetxController {
  final inProgressBookings = <dynamic>[].obs;
  ...
  Future<void> fetchInProgressBookings() async {
    try {
      ...
      log('In Progress Screen API Response: ${response.statusCode}');
  ...
  void navigateToSubmitWorkForm(Map<String, dynamic> booking) {
    ...
    log("Navigating to submit work form for booking: $bookingId");
  ...
  RecentJobRequestStatusWidget buildInProgressBookingWidget(int index) {
    ...
    return RecentJobRequestStatusWidget(
      ...
      onTap: null, // As per your original — no card tap navigation
      submitWorkButtonOnTap: () => navigateToSubmitWorkForm(booking),
      messageButtonOnTap: () {
        log("Message button tapped for booking: $bookingId");
        // Add real chat logic here if needed
      },
