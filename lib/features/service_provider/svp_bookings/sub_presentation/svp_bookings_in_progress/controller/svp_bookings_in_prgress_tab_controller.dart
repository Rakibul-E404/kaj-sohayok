...
//   final hasError = false.obs;
//   final errorMessage = ''.obs;
//   final isFetchingFormData = false.obs;
//
//   final NetworkCaller _networkCaller = NetworkCaller();
//
//   @override
...
//           jobRequests.assignAll(results);
//
//           // Print booking IDs for debugging
//           print('\n📋 SERVICE BOOKING IDs LIST:');
//           print('===============================');
//           for (int i = 0; i < results.length; i++) {
//             final booking = results[i];
...
//
//     log("Navigating to job details for in-progress booking ID: $bookingId");
//
//     Get.toNamed(
//       Routes.svpJobDetailsScreen,
//       arguments: {
//         // "status": JobRequestStatusEnum.inProgress, // Use inProgress status
//         "bookingId": bookingId,
//         "jobRequest": jobRequest,
//         "userId": userId,
//       },
...
//     return RecentJobRequestStatusWidget(
//       isJobInProgress: true,
//       onTap: () => navigateToJobDetails(jobRequest), // Added: Tap to go to details
//       submitWorkButtonOnTap: () => navigateToSubmitWorkForm(jobRequest),
//       messageButtonOnTap: () {
//         log("Message button tapped for booking: $bookingId");
...
//       location: address,
//       dateTime: formatDateTime(bookingDateTime),
//       // Optionally show loading on the submit work button
//       // isSubmitWorkLoading: isFetchingFormData.value &&
//       //     jobRequest['_ServiceBookingId'] == bookingId,
//     );
//   }
...
class SvpBookingsInProgressController extends GetxController {
  ...
  void onReady() {
    super.onReady();
    print('✅ SvpBookingsInProgressController ready');
  }

  @override
  void onClose() {
    print('👋 SvpBookingsInProgressController closed');
    super.onClose();
  }

  // Method to set up reactive listener for printing booking IDs
  void setupBookingIdPrinter() {
    ever(jobRequests, (List<dynamic> newJobRequests) {
      if (newJobRequests.isNotEmpty) {
        printBookingIds(newJobRequests, 'LISTENER TRIGGERED');
      }
    });
  }

  // Method to print booking IDs in a formatted way
  void printBookingIds(List<dynamic> bookings, String triggerSource) {
    final timestamp = DateTime.now().toLocal();
    print('\n📋 SERVICE BOOKING IDs LIST - $triggerSource');
    ...
    print('==================================================');
    print('📊 Total: ${bookings.length} booking(s)');
    print('\n');
  }

  Future<void> fetchInProgressBookings() async {
    try {
      print('🔄 fetchInProgressBookings() called');
      isLoading.value = true;
      hasError.value = false;
      errorMessage.value = '';
      ...
      print('🌐 Calling API: ${AppUrl.providerInProgressBookings}');
      final NetworkResponse response = await _networkCaller.getRequest(
        AppUrl.providerInProgressBookings,
        headers: {'Authorization': 'Bearer $token'},
      );
      ...
      if (response.isSuccess && response.jsonResponse != null) {
        ...
        if (responseData['success'] == true &&
          ...
          // Print booking IDs immediately after assignment
          printBookingIds(results, 'API RESPONSE');
  ...
  Future<void> navigateToSubmitWorkForm(Map<String, dynamic> jobRequest) async {
    try {
      final bookingId = jobRequest['_ServiceBookingId'] as String? ?? '';
      print("📝 Fetching work form data for booking ID: $bookingId");
      ...
      print('📄 Work Form API Response: ${response.statusCode}');
      ...
      if (response.isSuccess && response.jsonResponse != null) {
        ...
        if (responseData['code'] == 200) {
          ...
          print("✅ Successfully loaded work form for booking ID: $bookingId");
  ...
  RecentJobRequestStatusWidget buildInProgressBookingWidget(int index) {
    ...
    return RecentJobRequestStatusWidget(
      ...
      submitWorkButtonOnTap: () => navigateToSubmitWorkForm(jobRequest),
      messageButtonOnTap: () {
        print("💬 Message button tapped for booking: $bookingId");
        // TODO: Implement messaging functionality
        Get.snackbar(
          'Message',
          'Messaging feature coming soon!',
