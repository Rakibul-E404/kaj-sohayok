...
class PaymentRequestBookingsController extends GetxController {
  // 🔴 FIXED: Use lazy getters instead of immediate initialization
  SvpHomeScreenController get svpHomeScreenController {
    try {
      return Get.find<SvpHomeScreenController>();
    } catch (e) {
      log('⚠️ [PAYMENT REQUEST CONTROLLER] SvpHomeScreenController not found, creating new instance');
      return Get.put(SvpHomeScreenController());
    }
  }

  SvpPaymentRequestController get svpPaymentRequestController {
    try {
      return Get.find<SvpPaymentRequestController>();
    } catch (e) {
      log('⚠️ [PAYMENT REQUEST CONTROLLER] SvpPaymentRequestController not found, creating new instance');
      return Get.put(SvpPaymentRequestController());
    }
  }

  SvpWorkCompletedController get svpWorkCompletedController {
    try {
      return Get.find<SvpWorkCompletedController>();
    } catch (e) {
      log('⚠️ [PAYMENT REQUEST CONTROLLER] SvpWorkCompletedController not found, creating new instance');
      return Get.put(SvpWorkCompletedController());
    }
  }
  ...
  @override
  void onInit() {
    log('🎯 [PAYMENT REQUEST CONTROLLER] Controller initialized');
    super.onInit();
    // Call after super.onInit() to ensure proper initialization
    getPaymentRequestBookings();
  }
  ...
  Future<void> getPaymentRequestBookings() async {
    try {
      ...
      // 🔴 FIXED: Use SecureStorageService directly like PendingController
      final token = await SecureStorageService().read(AppConstants.accessToken);
      log('🔑 [PAYMENT REQUEST CONTROLLER] Token retrieved: ${token != null ? "Yes (${token.length} chars)" : "No"}');
      ...
      log('🌐 [PAYMENT REQUEST CONTROLLER] Making API call to: ${AppUrl.paymentRequests}');
      log('🔑 [PAYMENT REQUEST CONTROLLER] Headers: ${headers.keys.join(", ")}');
      ...
      if (response.isSuccess && response.jsonResponse != null) {
        ...
        errorMessage.value = error;
        log('❌ [PAYMENT REQUEST CONTROLLER] Network error: $error');
        log('❌ [PAYMENT REQUEST CONTROLLER] Response body: ${response.jsonResponse}');
      }
    } catch (e, stackTrace) {
      errorMessage.value =
          'Connection error: Please check your internet connection';
      log('❌ [PAYMENT REQUEST CONTROLLER] Exception in getPaymentRequestBookings: $e');
      log('❌ [PAYMENT REQUEST CONTROLLER] Stack trace: $stackTrace');
    } finally {
      isLoading.value = false;
      log('🏁 [PAYMENT REQUEST CONTROLLER] Loading completed');
    }
